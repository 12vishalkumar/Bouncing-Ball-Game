...
score = Label(height=50, width=80, text="Your Score: 00", font="Consolas 14 bold")
...
class Ball:
    ...
    # ************************ Function to handle bricks of the game **********************
                ...
                try:
                    if pos[2] >= brick_pos[0] and pos[0] <= brick_pos[2]:
                        if pos[3] >= brick_pos[1] and pos[1] <= brick_pos[3]:
                            canvas.bell()
                            self.hit += 1
                            self.score.configure(text="Your Score: " + str(self.hit))
    ...
    # ******************************* Function to handle Paddle game **********************    
...
class Paddle:
    ...
    # ******************************* Function when game going to draw **************
    def draw(self):
        pos = self.canvas.coords(self.id)
        if pos[0] + self.x <= 0:
            self.x = 0
...
def start_game(event):
    ...
    if(playing is False):
        ...
        BRICK_COLOR = ["PeachPuff3", "dark slate gray", "rosy brown", "light goldenrod yellow", "turquoise3", 
                       "salmon", "light steel blue", "dark khaki", "pale violet red", "orchid", "tan", "MistyRose2",
        ...
        while 1:
            if(paddle.pausec != 1):
                ...
                if(not ball.bottom_hit):
                    ...
                    time.sleep(0.01)
                    if(ball.hit == 95):
                        canvas.create_text(250, 250, text="WOW!🎉🎊 YOU WING THE GAME", fill="yellow", font="Consolas 24")
                        root.update_idletasks()
                        root.update()
                        playing = False
                        break
                else:
                    canvas.create_text(250, 250, text="GAME IS OVER!", fill="red", font="Consolas 24")
                    root.update_idletasks()
                    root.update()
                    playing = False
                    break
                ...
                try:
                    ...
                    m = canvas.create_text(250, 250, text="PAUSE THE GAME!", fill="green", font="Consolas 24")
                root.update_idletasks()
                root.update()

# ********************************* Function Calls **************************************
root.bind_all("<Return>", start_game)
canvas.create_text(250, 250, text="Press Enter Button to Start Game!", fill="red", font="Consolas 18")
j = canvas.find_all()
root.mainloop()
